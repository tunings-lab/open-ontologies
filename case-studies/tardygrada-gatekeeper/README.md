...
```bash
./run-demo.sh     # pure Python standard library, no dependencies
```

## Honest scope

See [`BUILD_REPORT.md`](BUILD_REPORT.md). The guarantee is exhaustive over a *bounded* model
(two robots, a 3x3 grid, a small battery); scaling to large or unbounded systems needs symbolic /
SMT model checking, where the trusted checker stays small while the reachable space is handled
abstractly, this reference makes that checker explicit and measures it. The trusted core is
small (31 lines) but this repo does not ship a machine-checked proof of the checker itself; that
is the role of the [Tardygrada](https://github.com/fabio-rovai) formally-verified agent runtime,
whose C core is the production vehicle for a gate like this. The scenario is a reference model in
the spirit of the DeepMind AI Safety Gridworlds (irreversible side effects), not those exact
environments; running the gate on Gridworlds and Melting Pot is the documented next step.
